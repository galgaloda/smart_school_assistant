...
class SessionManager {
  ...
  static Future<void> _loadSessionData() async {
    try {
      // Check if box is already open, if not try to open it
      Box? sessionBox;
      if (Hive.isBoxOpen(_sessionBoxName)) {
        sessionBox = Hive.box(_sessionBoxName);
      } else {
        try {
          sessionBox = await Hive.openBox(_sessionBoxName);
        } catch (e) {
          print('[SESSION] Could not open session box for loading: $e');
          return;
        }
      }
    ...
    try {
      ...
      if (Hive.isBoxOpen(_sessionBoxName)) {
        ...
        try {
          ...
          print('[SESSION] Could not open session box for saving: $e');
          return;
        }
      }
    ...
    try {
      ...
      if (Hive.isBoxOpen(_sessionBoxName)) {
        ...
        try {
          ...
          print('[SESSION] Could not open session box: $e');
          // Continue with clearing in-memory session data
        }
      }
      ...
      // Ensure we still clear in-memory data even if storage fails
      user_role.AccessControlManager.setCurrentUser(null);
      _lastActivity = null;
    }
  }
